...
void read_customers(string filename) {
    ...
    while(std::getline(file, str)) {
        std::vector<string> pieces = split(str, ',');
...
void read_orders(string filename) {
    ...
    while(std::getline(file, str1) && std::getline(file, str2)) {
        std::vector<string> pieces1 = split(str1, ',');
        std::vector<string> pieces2 = split(str2, ',');
        ...
        for(int i = 3; i < pieces1.size(); i++) {
            std::vector<string> item_pieces = split(pieces1.at(i), '-');
